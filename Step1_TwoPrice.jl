...
@objective(S1_TwoPrice, Max, 
            sum( sum(PI[s]
            * ((f_DA[h,s] * w_da[h]) 
            + f_SB[h,s] * ( (0.9 * f_DA[h,s] * w_up[h,s]) - (f_DA[h,s] * w_dw[h,s])  ) #active when the system has power excess
            + abs((f_SB[h,s]-1)) * ( (f_DA[h,s] * w_up[h,s]) - (1.2 * f_DA[h,s] * w_dw[h,s]) ) #active when the system has power deficit
            ) 
            for s=1:S) for h=1:H)
            )
...
@constraint(S1_TwoPrice, [h=1:H,s=1:S], w_im[h,s] == f_WP[h,s] - w_da[h] ) #real time imbalance
