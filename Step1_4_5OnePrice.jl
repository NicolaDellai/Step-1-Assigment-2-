...
for s=1:400
    for h=1:24
        W_IM[h,s] = f_WP2[h,s] - W_DA14[h]
        if W_IM[h,s] >= 0
        ep14[h,s] =  f_DA2[h,s] * W_DA14[h] 
    ...
    EP14_out[s] = sum(ep14[h,s] for h=1:24)
end

EP14_out_avg = sum(EP14_out)/length(EP14_out)
...
savefig("Results/1_4_5_OnePrice.png")
