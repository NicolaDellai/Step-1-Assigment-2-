...
@objective(S1_OnePrice, Max, 
            sum( sum(PI[s]
            * ((f_DA[h,s] * w_da[h]) 
            + f_SB[h,s] * ( - (0.9 * f_DA[h,s] * w_up[h,s]) + (0.9 * f_DA[h,s] * w_dw[h,s]) ) #active when the system has power excess
            + abs((f_SB[h,s]-1)) * ( (1.2 * f_DA[h,s] * w_up[h,s]) - (1.2 * f_DA[h,s] * w_dw[h,s]) ) #active when the system has power deficit
            ) 
            for s=1:S) for h=1:H)
            )
...
@constraint(S1_OnePrice, [h=1:H,s=1:S], w_im[h,s] == f_WP[h,s] - w_da[h] ) #real time imbalance
...
println("Profit under the One Price scheme: $(round.(objective_value(S1_TwoPrice)))\$")
...
for s=1:S
    ...
    for h=1:H
    println("$(round(f_WP[h,s]))\t$(round(W_DA[h]))\t$(round(W_IM[h,s]))")
    end
end
